...
async function fetchReferrals() {
  ...
  try {
    ...
    const res = await apiFetch<{ data: Referral[]; meta: any }>(`/api/v1/team/referrals?${params}`)
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 pt-10 pb-32">
    ...
    <div class="flex items-center justify-between mb-8 flex-wrap gap-4">
      <div>
        ...
        <p class="text-[14px] mt-1" style="color: var(--color-text-secondary);">Work the referral programme — qualify leads and keep their status current.</p>
      </div>
    </div>
...
v-for="r in referrals" :key="r.id"
            ...
            @click="navigateTo(`/team/referrals/${r.id}`)">
