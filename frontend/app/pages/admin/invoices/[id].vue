...
<template>
  <div class="max-w-5xl mx-auto px-4 sm:px-6 pt-10 pb-32">
    <NuxtLink to="/admin/invoices" class="inline-flex items-center gap-2 text-[13px] mb-8 transition-opacity hover:opacity-70"
    ...
    <template v-else-if="invoice">
      ...
      <div class="flex items-start justify-between gap-4 flex-wrap mb-8">
        <div class="min-w-0">
          <div class="flex items-center gap-3 flex-wrap">
            ...
            <span v-if="invoice.is_overdue" class="text-[11px] font-semibold px-2 py-0.5 rounded-full"
        ...
        <a :href="invoice.pdf_path" target="_blank" rel="noopener"
      ...
      <div class="grid lg:grid-cols-3 gap-5">
        ...
        <div class="rounded-2xl border p-6"
          ...
          <div class="space-y-2 pt-4 border-t" style="border-color: var(--color-border);">
            <NuxtLink v-if="invoice.order_number" :to="`/admin/orders/${invoice.order_id}`"
      ...
      <!-- Payments allocated -->
      <div class="rounded-2xl border p-6 mt-5"
        :style="{ background: 'var(--color-bg-elevated)', borderColor: 'var(--color-border)' }">
