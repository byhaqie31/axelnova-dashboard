...
Mints the AXN document family — `AXN-{TYPE}-{YYYY}-{NNNN}`, type `Q` (quotation) / `O` (order) / `I` (invoice, future) via the [`DocumentType`](../../backend/app/Support/DocumentType.php) enum — atomically using a DB transaction with `lockForUpdate()`. Each type has its own counter that resets each year.
