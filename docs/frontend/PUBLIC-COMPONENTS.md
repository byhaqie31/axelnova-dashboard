...
- **The dwell is a timed scroll lock after the pin, not pin distance** (`startDwell`): on first arrival at the composed view it calls `lenis.stop()` for `DWELL_MS`, then releases. It self-kills so it can only ever fire once — a re-firing scroll lock reads as a broken page. Two guards matter: `programmaticScroll` stops the dwell from grabbing scroll mid-`scrollTo` (which would abort the chip's animation and strand the page short of target, so that path fires the dwell from `onComplete`), and the safety timer checks `dwelling` before releasing lenis.
