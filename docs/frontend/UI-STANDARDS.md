...
- **`StatusPill`** — status badge with semantic tone mapping. Props: `status`, `type` (`lead` | `quotation` | `project` | `invoice` | `milestone` | `referral` | `referral_partner` | `task` | `user` | `feedback`). Reads existing CSS tokens (`--color-accent`, `--color-success`, `--color-warning`, `--color-danger`); no new color tokens introduced. `referral` / `referral_partner` were added for the `/admin/referrals` hub (Task 2 of the portal restructure) — first real adoption of this primitive; earlier admin pages still use the older `AdminStatusPill.vue` (data-attribute driven, `main.css` tokens) and haven't been migrated. `task` (Task 5 — the tasks engine) maps the workflow spine: `open` neutral, `in_progress` info, `completed`/`paid` success, `payment_pending` warn. `user` (Task 8 — `/admin/users`) is derived client-side from `deactivated_at`, not a stored enum: `active` success, `deactivated` danger. Role itself (founder/marketer/engineer) is a separate chip built from `data/workspaceRoles.ts`, not this primitive — founder gets a distinct warning/gold tone + crown icon. `feedback` (the Feedback & Reviews module — see [FEEDBACK-MODULE.md](../global/FEEDBACK-MODULE.md)) maps the review lifecycle: `pending` warn, `approved` info (accent), `published` success, `archived` neutral/muted; the option list lives in `data/feedbackStatuses.ts` alongside the NPS-bucket chip tones.
...
> ✅ **Resolved.** `--color-switch-off-track` is now defined in `main.css` (light `#d1d5db`, dark `rgba(255,255,255,0.18)`) — new toggles (feedback module onward) consume it; older forms still carry the literal and can migrate opportunistically.

> ✅ **Resolved.** `--color-on-accent: #FFFFFF` and `--color-on-success: #FFFFFF` are now defined in `main.css` for foregrounds on filled accent/success surfaces. Existing `var(--color-on-accent, #fff)` fallbacks now resolve to the token.
